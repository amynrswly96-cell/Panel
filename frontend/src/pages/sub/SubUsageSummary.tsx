...
export default function SubPage() {
  ...
  const cardTitle = (
    <Space size={10} className="term-title">
      ...
      <EagleLogo size={18} />
      <span className="term-path hk-glow-text">root@subscription:~$</span>
      <Tag className="sid-tag">{sId}</Tag>
    </Space>
  );
        ...
        <Layout.Content className="content">
          <Row justify="center">
            <Col xs={24} sm={22} md={18} lg={14} xl={12}>
              <Card hoverable className="subscription-card term-window" title={cardTitle} extra={cardExtra}>
                {announce && (
                  <Alert type="info" showIcon title={announce} style={{ marginBottom: 16 }} />
                )}

                <div className="term-block info-table">
                  <div className="term-block-label">// account_info</div>
                  <div className="term-kv-grid">
                    {descriptionsItems.map((item) => (
                      <div className="term-kv-row" key={item.key}>
                        <span className="term-kv-key">{item.label}</span>
                        <span className="term-kv-val">{item.children}</span>
                      </div>
                    ))}
                  </div>
                </div>
